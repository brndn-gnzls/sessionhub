...
GraphQLScalarLiteralParser = Callable[[ValueNode, dict[str, Any] | None], Any]
...
class GraphQLScalarType(GraphQLNamedType):
    ...
    def parse_literal(self, node: ValueNode, variables: dict[str, Any] | None = None) -> Any:
...
class GraphQLInputObjectType(GraphQLNamedType):
    ...
    def fields(self) -> GraphQLInputFieldMap:
        ...
        return {
            assert_name(name): (
                value if isinstance(value, GraphQLInputField) else GraphQLInputField(value)  # type: ignore
            )
            for name, value in fields.items()
        }
