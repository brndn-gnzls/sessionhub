...
class KnownArgumentNamesOnDirectivesRule(ASTValidationRule):
    ...
    def __init__(self, context: ValidationContext | SDLValidationContext):
                ...
                if arg_name not in known_args:
                    ...
                    self.report_error(
                        GraphQLError(
                            ...
                            f" on directive '@{directive_name}'." + did_you_mean(suggestions),
                            arg_node,
                        )
                    )
        return SKIP
    ...
    def enter_argument(self, arg_node: ArgumentNode, *args: Any) -> None:
        ...
        if not arg_def and field_def and parent_type:
            ...
            context.report_error(
                GraphQLError(
                    f"Unknown argument '{arg_name}'"
                    f" on field '{parent_type.name}.{field_name}'." + did_you_mean(suggestions),
                    arg_node,
                )
            )
