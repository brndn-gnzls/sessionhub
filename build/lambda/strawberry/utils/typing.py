...
def is_concrete_generic(annotation: type) -> bool:
    ...
    return isinstance(annotation, _GenericAlias) and annotation.__origin__ not in ignored_generics
...
def _ast_replace_union_operation(
    ...
    if isinstance(expr, ast.Expr) and isinstance(expr.value, (ast.BinOp, ast.Subscript)):
...
def _get_namespace_from_ast(
    ...
    if isinstance(expr, ast.Expr) and isinstance(expr.value, (ast.BinOp, ast.Subscript)):
        ...
        for arg in args[1:]:
            evaled_arg = eval(arg, globalns, localns)
...
def eval_type(
    ...
    if origin is not None:
        ...
        if origin is Annotated:
            for arg in args[1:]:
                ...
                if isinstance(arg, StrawberryLazyReference):
                    remaining_args = [
                        a for a in args[1:] if not isinstance(a, StrawberryLazyReference)
                    ]
                    type_arg = (
                ...
                if isinstance(arg, StrawberryAuto):
                    remaining_args = [a for a in args[1:] if not isinstance(a, StrawberryAuto)]
                    args = (args[0], arg, *remaining_args)
                    break

                if isinstance(arg, StrawberryParent):
                    remaining_args = [a for a in args[1:] if not isinstance(a, StrawberryParent)]
                    try:
                        type_arg = (
        ...
        type_ = origin[tuple(eval_type(a, globalns, localns) for a in args)] if args else origin
