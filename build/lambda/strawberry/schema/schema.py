...
class Schema(BaseSchema):
    def __init__(
        ...
        scalar_overrides: Optional[
            Mapping[object, type | ScalarWrapper | ScalarDefinition],
        ] = None,
        ...
        mutation_type = (
            self.schema_converter.from_object(
                cast("type[WithStrawberryObjectDefinition]", mutation).__strawberry_definition__
            )
            if mutation
            else None
        )
        subscription_type = (
            self.schema_converter.from_object(
                cast("type[WithStrawberryObjectDefinition]", subscription).__strawberry_definition__
            )
            if subscription
            else None
        )
        ...
        for type_ in types:
            if compat.is_schema_directive(type_):
                graphql_directives.append(self.schema_converter.from_schema_directive(type_))
            else:
                if (
    ...
    def _get_middleware_manager(self, extensions: list[SchemaExtension]) -> MiddlewareManager:
        ...
        try:
            with extensions_runner.operation():
                ...
                try:
                    ...
                    raise CannotGetOperationTypeError(execution_context.operation_name) from error
    ...
    async def _subscribe(
        ...
        async with extensions_runner.operation():
            ...
            # catch exceptions raised in `on_execute` hook.
                ...
                origin_result = OriginalExecutionResult(data=None, errors=[_coerce_error(exc)])
                yield await self._handle_execution_result(
                    execution_context,
                    origin_result,
                    extensions_runner,
    ...
    def _warn_for_federation_directives(self) -> None:
        ...
        all_directives = (
            directive for type_def in all_type_defs for directive in (type_def.directives or [])
        )

        if any(isinstance(directive, FederationDirective) for directive in all_directives):
            warnings.warn(
