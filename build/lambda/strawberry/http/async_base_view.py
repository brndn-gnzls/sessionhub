...
class AsyncWebSocketAdapter(abc.ABC):
    ...
    @abc.abstractmethod
    def iter_json(self, *, ignore_parsing_errors: bool = False) -> AsyncGenerator[object, None]: ...
...
class AsyncBaseHTTPView(
    ...
    graphql_ws_handler_class: type[BaseGraphQLWSHandler[Context, RootValue]] = BaseGraphQLWSHandler[
        Context, RootValue
    ]
    ...
    async def run(
        ...
        context = (
            await self.get_context(request, response=sub_response) if context is UNSET else context
        )
        ...
        if isinstance(result, GraphQLIncrementalExecutionResults):
            ...
            async def stream() -> AsyncGenerator[str, None]:
                ...
                response["pending"] = [p.formatted for p in result.initial_result.pending]
                ...
                async for value in result.subsequent_results:
                    ...
                    if value.incremental:
                        ...
                        for incremental_value in value.incremental:
                            pending_value = next(
                                (v for v in all_pending if v.id == incremental_value.id),
                                None,
                            )
