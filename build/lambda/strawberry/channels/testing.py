...
class GraphQLWebsocketCommunicator(WebsocketCommunicator):
    ...
    async def gql_init(self) -> None:
        ...
        if self.protocol == GRAPHQL_TRANSPORT_WS_PROTOCOL:
            ...
            transport_ws_connection_ack_message: transport_ws_types.ConnectionAckMessage = (
            ...
            await self.send_json_to(ws_types.ConnectionInitMessage({"type": "connection_init"}))
    ...
    async def subscribe(
        ...
        while True:
            message: transport_ws_types.Message = await self.receive_json_from(timeout=5)
            if message["type"] == "next":
                ...
                yield ExecutionResult(data=None, errors=self.process_errors(error_payload))
