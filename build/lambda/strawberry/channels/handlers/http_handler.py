...
from typing import TYPE_CHECKING, Any, TypeGuard, Union, assert_never
...
class ChannelsRequest:
    ...
    def form_data(self) -> FormData:
        upload_handlers = [
            uploadhandler.load_handler(handler) for handler in settings.FILE_UPLOAD_HANDLERS
        ]
...
class GraphQLHTTPConsumer(
    ...
    async def create_streaming_response(
        ...
        response_headers = {k.encode(): v.encode() for k, v in sub_response.headers.items()}
        response_headers.update({k.encode(): v.encode() for k, v in headers.items()})

        return MultipartChannelsResponse(stream=stream, status=status, headers=response_headers)
