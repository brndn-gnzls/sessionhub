...
class StrawberryType(ABC):
    ...
    def copy_with(
        self,
        type_var_map: Mapping[str, StrawberryType | type[WithStrawberryObjectDefinition]],
...
class StrawberryObjectDefinition(StrawberryType):
    ...
    type_var_map: Mapping[str, StrawberryType | type] = dataclasses.field(default_factory=dict)
    ...
    def resolve_generic(self, wrapped_cls: type) -> type:
        ...
        type_var_map = dict(zip((param.__name__ for param in params), resolved_types, strict=False))
    ...
    def get_field(self, python_name: str) -> StrawberryField | None:
        return next((field for field in self.fields if field.python_name == python_name), None)
    ...
    def is_specialized_generic(self) -> bool:
        return self.is_graphql_generic and not getattr(self.origin, "__parameters__", None)
