...
class StrawberryUnion(StrawberryType):
    ...
    def types(self) -> tuple[StrawberryType, ...]:
        return tuple(
            cast("StrawberryType", annotation.resolve()) for annotation in self.type_annotations
        )
    ...
    def type_params(self) -> list[TypeVar]:
        ...
        return list(set(itertools.chain(*(_get_type_params(type_) for type_ in self.types))))
    ...
    def get_type_resolver(self, type_map: TypeMap) -> GraphQLTypeResolver:
        def _resolve_union_type(
            ...
            for possible_concrete_type in chain(concrete_types_for_union, type_map.values()):
