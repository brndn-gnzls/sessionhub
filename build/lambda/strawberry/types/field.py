...
class StrawberryField(dataclasses.Field):
    ...
    def __init__(
        ...
        if len(self.permission_classes):
            ...
            if not self.extensions:
                self.extensions = []
            permission_instances = [permission_class() for permission_class in permission_classes]
            # Append to make it run first (last is outermost)
            self.extensions.append(PermissionExtension(permission_instances, use_directives=False))
    ...
    def __copy__(self) -> Self:
        ...
        new_field._arguments = self._arguments[:] if self._arguments is not None else None
        return new_field
    ...
    def is_graphql_generic(self) -> bool:
        return (
            self.base_resolver.is_graphql_generic if self.base_resolver else _is_generic(self.type)
        )
    ...
    def type(self, type_: Any) -> None:
        ...
        self.type_annotation = StrawberryAnnotation.from_annotation(type_, namespace=None)
    ...
    def copy_with(self, type_var_map: Mapping[str, StrawberryType | builtins.type]) -> Self:
        ...
        if override_type is not None:
            new_field.type_annotation = StrawberryAnnotation(
                override_type,
                namespace=(self.type_annotation.namespace if self.type_annotation else None),
            )
