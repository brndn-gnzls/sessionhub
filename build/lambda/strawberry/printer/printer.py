...
def print_field_directives(
    ...
    return "".join(
        print_schema_directive(directive, schema=schema, extras=extras) for directive in directives
    )
...
def print_enum(type_: GraphQLEnumType, *, schema: BaseSchema, extras: PrintExtras) -> str:
    ...
    return (
        print_description(type_) + f"enum {type_.name}" + printed_directives + print_block(values)
    )
...
def print_extends(type_: GraphQLObjectType, schema: BaseSchema) -> str:
    ...
    strawberry_type = cast(
        ...
        type_.extensions and type_.extensions.get(GraphQLCoreConverter.DEFINITION_BACKREF),
    )
...
def print_type_directives(
    ...
    allowed_locations = [Location.INPUT_OBJECT] if strawberry_type.is_input else [Location.OBJECT]
...
def _all_root_names_are_common_names(schema: BaseSchema) -> bool:
    ...
    subscription = schema.subscription.__strawberry_definition__ if schema.subscription else None
...
def print_schema(schema: BaseSchema) -> str:
    ...
    types = [
        type_ for type_name in sorted(type_map) if is_defined_type(type_ := type_map[type_name])
    ]
    ...
    return "\n\n".join(
        chain(
            ...
            types_printed,
            (
                _print_type(schema.schema_converter.from_type(type_), schema, extras=extras)
