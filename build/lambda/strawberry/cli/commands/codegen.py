...
def _import_plugin(plugin: str) -> type[QueryCodegenPlugin] | None:
    ...
    symbols = {key: value for key, value in module.__dict__.items() if not key.startswith("__")}

    if "__all__" in module.__dict__:
        symbols = {
            name: symbol for name, symbol in symbols.items() if name in module.__dict__["__all__"]
        }
...
def _load_plugins(plugin_ids: list[str], query: Path) -> list[QueryCodegenPlugin | ConsolePlugin]:
    plugins = []
...
def codegen(
    query: list[Path] | None = typer.Argument(default=None, exists=True, dir_okay=False),
