...
class BaseGraphQLWSHandler(Generic[Context, RootValue]):
    ...
    async def handle(self) -> None:
        try:
            try:
                ...
                await self.websocket.close(code=1002, reason="WebSocket message type must be text")
    ...
    async def handle_connection_init(self, message: ConnectionInitMessage) -> None:
        ...
        if isinstance(connection_ack_payload, UnsetType) or connection_ack_payload is None:
            await self.send_message({"type": "connection_ack"})
        else:
            await self.send_message({"type": "connection_ack", "payload": connection_ack_payload})
    ...
    async def handle_start(self, message: StartMessage) -> None:
        ...
        result_handler = self.handle_async_results(operation_id, query, operation_name, variables)
    ...
    async def send_data_message(self, execution_result: ExecutionResult, operation_id: str) -> None:
        ...
        if execution_result.errors:
            data_message["payload"]["errors"] = [err.formatted for err in execution_result.errors]
