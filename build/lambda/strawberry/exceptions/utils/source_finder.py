...
class LibCSTSourceFinder:
    ...
    def _find_class_definition(self, source: SourcePath, cls: type[Any]) -> CSTNode | None:
        import libcst.matchers as m
                ...
                m.SubscriptElement(
                    slice=m.Index(
                        value=m.Call(
                            ...
                            args=[
                                m.Arg(
                                    value=m.SimpleString(value=f"'{union_definition.graphql_name}'")
                                    | m.SimpleString(value=f'"{union_definition.graphql_name}"')
                                )
                            ],
                        )
                    )
