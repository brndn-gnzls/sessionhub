...
class PermissionFailSilentlyRequiresOptionalError(StrawberryException):
    def __init__(self, field: StrawberryField) -> None:
        self.field = field
        self.message = "Cannot use fail_silently=True with a non-optional or non-list field"
        self.rich_message = (
    ...
    def exception_source(self) -> ExceptionSource | None:
        ...
        if source is None and self.field.base_resolver is not None:
            source = source_finder.find_function_from_object(self.field.base_resolver.wrapped_func)
