...
class UpdateImportsCodemod(VisitorBasedCodemodCommand):
    ...
    def _update_imports(self, node: cst.ImportFrom, updated_node: cst.ImportFrom) -> cst.ImportFrom:
        imports = [
            "field",
            "union",
            "auto",
        ...
        for import_name in imports:
            if m.matches(
                node,
                m.ImportFrom(
                    module=m.Attribute(value=m.Name("strawberry"), attr=m.Name(import_name))
                ),
            ):
                updated_node = updated_node.with_changes(
                    module=cst.Attribute(
                        value=cst.Attribute(value=cst.Name("strawberry"), attr=cst.Name("types")),
    ...
    def _update_strawberry_type_imports(
        ...
        if m.matches(
            node,
            m.ImportFrom(module=m.Attribute(value=m.Name("strawberry"), attr=m.Name("type"))),
        ):
