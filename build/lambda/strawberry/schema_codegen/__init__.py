...
class Import:
    ...
    def to_cst(self) -> cst.Import | cst.ImportFrom:
        if self.module is None:
            return cst.Import(names=[cst.ImportAlias(name=cst.Name(name)) for name in self.imports])
...
def _get_field_type(field_type: TypeNode, was_non_nullable: bool = False) -> cst.BaseExpression:
...
def _sanitize_argument(value: ArgumentValue) -> cst.SimpleString | cst.Name | cst.List:
    ...
    if isinstance(value, list):
        return cst.List(
            elements=[
                cst.Element(value=_sanitize_argument(item)) for item in value if item is not None
            ],
        )
...
def _get_federation_arguments(
    ...
    append_arg_from_directive("requiresScopes", "scopes", "requires_scopes", flatten=False)
    ...
    arguments.extend(_get_argument(key, True) for key in boolean_keys if directives.get(key, False))
...
def _get_strawberry_decorator(
    ...
    description = (
        definition.description if not isinstance(definition, ObjectTypeExtensionNode) else None
    )
...
def _get_class_definition(
    ...
    class_definition = cst.ClassDef(
        ...
        body=cst.IndentedBlock(
            body=[_get_field(field, is_apollo_federation, imports) for field in definition.fields]
        ),
...
def _get_enum_definition(definition: EnumTypeDefinitionNode) -> Definition:
    ...
    class_definition = cst.ClassDef(
        ...
        body=cst.IndentedBlock(body=[_get_enum_value(value) for value in definition.values]),
        decorators=[decorator],
    )
...
def _get_union_definition(definition: UnionTypeDefinitionNode) -> Definition:
    ...
    types = cst.parse_expression(" | ".join([type_.name.value for type_ in definition.types]))
...
def _get_scalar_definition(
    ...
    additional_args: list[cst.Arg | None] = [
        _get_argument("description", description) if description else None,
        _get_argument("specified_by_url", specified_by_url) if specified_by_url else None,
        cst.Arg(
...
def codegen(schema: str) -> str:
    ...
    for graphql_definition in document.definitions:
        ...
        if isinstance(
            ...
            definition = _get_class_definition(graphql_definition, is_apollo_federation, imports)
    ...
    if root_subscription_name is None:
        root_subscription_name = "Subscription" if "Subscription" in definitions else None
