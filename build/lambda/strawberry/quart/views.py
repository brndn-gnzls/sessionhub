...
class QuartWebSocketAdapter(AsyncWebSocketAdapter):
    def __init__(self, view: AsyncBaseHTTPView, request: Websocket, response: Response) -> None:
        super().__init__(view)
        self.ws = request
...
class GraphQLView(
    AsyncBaseHTTPView[Request, Response, Response, Websocket, Response, Context, RootValue],
    View,
):
    ...
    async def dispatch_request(self, **kwargs: object) -> "ResponseReturnValue":
        try:
            return await self.run(request=websocket if has_websocket_context() else request)
