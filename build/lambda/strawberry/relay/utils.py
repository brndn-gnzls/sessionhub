...
def should_resolve_list_connection_edges(info: Info) -> bool:
    ...
    def _check_selection(selection: Selection) -> bool:
        ...
        """
        if not isinstance(selection, InlineFragment) and selection.name in resolve_for_field_names:
            return True
        if selection.selections:
            return any(_check_selection(selection) for selection in selection.selections)
        return False
