...
class PythonPlugin(QueryCodegenPlugin):
    ...
    def _get_type_name(self, type_: GraphQLType) -> str:
        ...
        if isinstance(type_, GraphQLScalar) and type_.name in self.SCALARS_TO_PYTHON_TYPES:
    ...
    def _print_argument_value(self, argval: GraphQLArgumentValue) -> str:
        if hasattr(argval, "values"):
            if isinstance(argval.values, list):
                return "[" + ", ".join(self._print_argument_value(v) for v in argval.values) + "]"
            if isinstance(argval.values, dict):
                return (
                    "{"
                    + ", ".join(
                        f"{k!r}: {self._print_argument_value(v)}" for k, v in argval.values.items()
                    )
                    + "}"
                )
    ...
    def _print_object_type(self, type_: GraphQLObjectType) -> str:
        fields = "\n".join(
            self._print_field(field) for field in type_.fields if field.name != "__typename"
        )
        ...
        if type_.graphql_typename:
            lines.append(textwrap.indent(f"# typename: {type_.graphql_typename}", indent))
    ...
    def _print_scalar_type(self, type_: GraphQLScalar) -> str:
        ...
        assert type_.python_type is not None, f"Scalar type must have a python type: {type_.name}"
