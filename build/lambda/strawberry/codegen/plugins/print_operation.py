...
class PrintOperationPlugin(QueryCodegenPlugin):
    ...
    def _print_graphql_type(self, type: GraphQLType, parent_type: GraphQLType | None = None) -> str:
    ...
    def _print_argument_value(self, value: GraphQLArgumentValue) -> str:
        ...
        if isinstance(value, GraphQLObjectValue):
            return (
                "{"
                + ", ".join(
                    f"{name}: {self._print_argument_value(v)}" for name, v in value.values.items()
                )
                + "}"
            )
