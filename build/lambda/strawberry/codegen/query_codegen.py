...
class ConsolePlugin:
    ...
    def on_start(self, plugins: Iterable[QueryCodegenPlugin], query: Path) -> None:
        ...
        rich.print(
            f"[green]Generating code for {query} using " f"{', '.join(plugin_names)} plugin(s)",
        )
...
class QueryCodegen:
    ...
    def _convert_value(self, value: ValueNode) -> GraphQLArgumentValue:
        ...
        if isinstance(value, ObjectValueNode):
            return GraphQLObjectValue(
                {field.name.value: self._convert_value(field.value) for field in value.fields}
            )
    ...
    def _collect_types(
        ...
        parent_type: StrawberryObjectDefinition,
        class_name: str,
        graph_ql_object_type_factory: Callable[[str], GraphQLObjectType] = GraphQLObjectType,
    ) -> GraphQLType:
        assert selection.selection_set is not None
