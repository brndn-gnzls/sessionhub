    ...
    def get_hook(self, extension: SchemaExtension) -> WrappedHook | None:
        ...
        if hook_fn:
            if inspect.isgeneratorfunction(hook_fn):
                context_manager = contextlib.contextmanager(types.MethodType(hook_fn, extension))
                return WrappedHook(extension=extension, hook=context_manager, is_async=False)
            ...
            if inspect.isasyncgenfunction(hook_fn):
                ...
                return WrappedHook(extension=extension, hook=context_manager_async, is_async=True)

            if callable(hook_fn):
                return self.from_callable(extension, hook_fn)

            raise ValueError(
                f"Hook {self.HOOK_NAME} on {extension} " f"must be callable, received {hook_fn!r}"
            )
