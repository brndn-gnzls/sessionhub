...
class OpenTelemetryExtension(SchemaExtension):
    ...
    def on_operation(self) -> Generator[None, None, None]:
        ...
        span_name = (
            f"GraphQL Query: {self._operation_name}" if self._operation_name else "GraphQL Query"
        )
    ...
    def convert_dict_to_allowed_types(self, value: dict) -> str:
        return (
            "{"
            + ", ".join(f"{k}: {self.convert_to_allowed_types(v)}" for k, v in value.items())
            + "}"
        )
    ...
    async def resolve(
        ...
        **kwargs: Any,
        ...
        with self._tracer.start_as_current_span(
            ...
            context=trace.set_span_in_context(self._span_holder[LifecycleStep.OPERATION]),
        ) as span:
