...
class TestCase(unittest.TestCase, metaclass=TestCaseMeta):
    ...
    def tearDown(self):
        ...
        for exc in self.__unhandled_exceptions:
            if isinstance(exc, ConnectionResetError):
                texc = traceback.TracebackException.from_exception(exc, lookup_lines=False)
        ...
        try:
            ...
            if elapsed > delta:
                raise AssertionError(
                    f"running block took {elapsed:0.3f}s which is longer "
                    f"than the expected maximum of {delta:0.3f}s"
                )
    ...
    def assertLoopErrorHandlerCalled(self, msg_re: str):
        ...
        try:
            ...
            raise AssertionError(
                f"no message matching {msg_re!r} was logged with " "loop.call_exception_handler()"
            )
        ...
        for key in sorted(context):
            ...
            if key == "source_traceback":
                ...
                try:
                    ...
                    value = f"Exception in __repr__ {ex!r}; " f"value type: {type(value)!r}"
