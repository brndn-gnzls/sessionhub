...
def _read_password_file(passfile: pathlib.Path) -> list[tuple[str, ...]]:
    ...
    try:
        ...
        if not passfile.is_file():
            warnings.warn(f"password file {passfile!r} is not a plain file")
...
def _read_password_from_pgpass(
    *, passfile: pathlib.Path | None, hosts: list[str], ports: list[int], database: str, user: str
):
    ...
    for host, port in zip(hosts, ports, strict=False):
        if host.startswith("/"):
...
def _dot_postgresql_path(filename) -> pathlib.Path | None:
    try:
...
def _parse_connect_dsn_and_args(
    *,
    ...
    if dsn:
        ...
        if parsed.scheme not in {"postgresql", "postgres"}:
            raise exceptions.ClientConfigurationError(
                ...
                f'"postgresql" or "postgres", got {parsed.scheme!r}'
            )
    ...
    for h, p in zip(host, port, strict=False):
        if h.startswith("/"):
            # UNIX socket name
            if ".s.PGSQL." not in h:
                h = os.path.join(h, f".s.PGSQL.{p}")
            addrs.append(h)
        else:
            # TCP host/port
            addrs.append((h, p))
    ...
    if isinstance(ssl, (str, SSLMode)):
        try:
            ...
            raise exceptions.ClientConfigurationError(
                f"`sslmode` parameter must be one of: {modes}"
            ) from None
    ...
    try:
        ...
        raise exceptions.ClientConfigurationError(
            ...
            f"{SessionAttribute.__members__.values!r}"
            f", got {target_session_attrs!r}"
        ) from None
    ...
    if gsslib not in {"gssapi", "sspi"}:
        raise exceptions.ClientConfigurationError(
            "gsslib parameter must be either 'gssapi' or 'sspi'" f", got {gsslib!r}"
        )
    ...
    *,
    ...
    for var_name in {
        ...
        if var_val is None or isinstance(var_val, bool) or var_val < 0:
            raise ValueError(
                f"{var_name} is expected to be greater " f"or equal to 0, got {var_val!r}"
            )
    ...
    def data_received(self, data):
        if data == b"S":
            ...
            self.on_data.set_exception(
                ConnectionError(
                    f'PostgreSQL server at "{self.host}:{self.port}" ' "rejected SSL upgrade"
                )
            )
...
async def _connect(*, loop, connection_class, record_class, **kwargs):
    ...
    raise last_error or exceptions.TargetServerAttributeNotMatched(
        "None of the hosts match the target attribute requirement " f"{target_attr!r}"
    )
...
def _get_socket(transport):
    ...
    if sock is None:
        ...
        raise ConnectionError(f"could not get the socket for transport {transport!r}")
    return sock
