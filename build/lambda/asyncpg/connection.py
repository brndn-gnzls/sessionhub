...
class Connection(metaclass=ConnectionMeta):
    ...
    def __del__(self):
        if not self.is_closed() and self._protocol is not None:
            if self._source_traceback:
                msg = f"unclosed connection {self!r}; created at:\n {self._source_traceback}"
            else:
                msg = (
                    f"unclosed connection {self!r}; run in asyncio debug "
        ...
        while types_with_missing_codecs:
            ...
            if tries > 5:
                ...
                raise exceptions.InternalClientError(
                    "could not resolve query result and/or argument types " f"in {tries} attempts"
                )
    ...
    async def copy_from_table(
        ...
        *,
        ...
        copy_stmt = f"COPY {tabname}{cols} TO STDOUT {opts}"
    ...
    async def copy_from_query(
        ...
        *args,
        ...
        copy_stmt = f"COPY ({query}) TO STDOUT {opts}"
    ...
    async def copy_to_table(
        ...
        *,
        ...
        copy_stmt = f"COPY {tabname}{cols} FROM STDIN {opts} {cond}"
    ...
    async def copy_records_to_table(
        ...
        intro_query = f"SELECT {col_list} FROM {tabname} LIMIT 1"
    ...
    async def set_builtin_type_codec(self, typename, *, schema="public", codec_name, format=None):
        ...
        if not introspection.is_scalar_type(typeinfo):
            raise exceptions.InterfaceError(f"cannot alias non-scalar type {schema}.{typename}")
        ...
        if self._protocol.is_in_transaction() or self._top_xact is not None:
            if self._top_xact is None or not self._top_xact._managed:
                ...
                self._loop.call_exception_handler(
                    {"message": "Resetting connection with an " f"active transaction {self!r}"}
                )
    ...
    def _get_unique_id(self, prefix):
        global _uid
        _uid += 1
        return f"__asyncpg_{prefix}_{_uid:x}__"
    ...
    *,
    ...
    if not issubclass(connection_class, Connection):
        raise exceptions.InterfaceError(
            ...
            f"asyncpg.Connection, got {connection_class!r}"
        )
...
class _Callback(typing.NamedTuple):
    ...
    def from_callable(cls, cb: typing.Callable[..., None]) -> "_Callback":
        if inspect.iscoroutinefunction(cb):
            ...
            raise exceptions.InterfaceError(
                "expected a callable or an `async def` function," f"got {cb!r}"
            )
...
def _check_record_class(record_class):
    if record_class is protocol.Record:
        ...
        raise exceptions.InterfaceError(
            "record_class is expected to be a subclass of " f"asyncpg.Record, got {record_class!r}"
        )
