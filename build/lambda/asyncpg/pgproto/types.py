...
class BitString:
    ...
    def __init__(self, bitstring: builtins.bytes | None = None) -> None:
        if not bitstring:
            ...
            for i, bit in enumerate(bitstring):
                ...
                bit = int(bit)
                if bit != 0 and bit != 1:
                    raise ValueError(f"invalid bit value at position {i}")
    ...
    @classmethod
    def frombytes(
        cls: type[_BitString], bytes_: builtins.bytes | None = None, bitlength: int | None = None
    ) -> _BitString:
    ...
    def __repr__(self) -> str:
        return f"<BitString {self.as_string()}>"
...
class Point(tuple[float, float]):
    ...
    def __new__(
        cls,
        x: typing.SupportsFloat | SupportsIndex | str | builtins.bytes | builtins.bytearray,
        y: typing.SupportsFloat | SupportsIndex | str | builtins.bytes | builtins.bytearray,
    ) -> "Point":
        return super().__new__(cls, typing.cast(typing.Any, (float(x), float(y))))

    def __repr__(self) -> str:
        return f"{type(self).__module__}.{type(self).__name__}({tuple.__repr__(self)})"
...
class Path:
    ...
    def __getitem__(self, i: int | slice) -> Point | tuple[Point, ...]:
