...
class Cluster:
    ...
    def get_status(self):
        ...
        if (
            ...
            raise ClusterError(f"pg_ctl status exited with status {process.returncode:d}: {stderr}")
    ...
    def init(self, **settings):
        ...
        if self.get_status() != "not-initialized":
            raise ClusterError(f"cluster in {self._data_dir!r} has already been initialized")
        ...
        if process.returncode != 0:
            raise ClusterError(
                f"pg_ctl init exited with status {process.returncode:d}:\n{output.decode()}"
            )
    ...
    def start(self, wait=60, *, server_settings={}, **opts):
        ...
        if status == "running":
            ...
            raise ClusterError(f"cluster in {self._data_dir!r} has not been initialized")
        ...
        if _system == "Windows":
            ...
            if process.returncode != 0:
                if process.stderr:
                    ...
                    stderr = ""
                raise ClusterError(
                    f"pg_ctl start exited with status {process.returncode:d}{stderr}"
                )
        else:
            if os.getenv("ASYNCPG_DEBUG_SERVER"):
                stdout = sys.stdout
    ...
    def reload(self):
        ...
        if process.returncode != 0:
            raise ClusterError(
                f"pg_ctl stop exited with status {process.returncode:d}: {stderr.decode()}"
            )
    ...
    def reset_wal(self, *, oid=None, xid=None):
        ...
        if process.returncode != 0:
            raise ClusterError(
                f"pg_resetwal exited with status {process.returncode:d}: {stderr.decode()}"
            )
    ...
    def add_hba_entry(
        ...
        if type not in {"local", "host", "hostssl", "hostnossl"}:
            raise ValueError(f"invalid HBA record type: {type!r}")
        ...
        if type != "local":
            if address is None:
                raise ValueError(f"{type!r} entry requires a valid address")
            else:
                record += f" {address}"
    ...
    def _run_pg_config(self, pg_config_path):
        ...
        if process.returncode != 0:
            raise ClusterError(f"pg_config exited with status {process.returncode:d}: {stderr}")
        else:
            config = {}
    ...
    def _find_pg_config(self, pg_config_path):
        ...
        if not os.path.isfile(pg_config_path):
            raise ClusterError(f"{pg_config_path!r} is not an executable")
    ...
    def _find_pg_binary(self, binary):
        ...
        if not os.path.isfile(bpath):
            raise ClusterError(
                ...
                + f"{bpath!r} does not exist or is not a file"
            )
    ...
    def _get_pg_version(self):
        ...
        if process.returncode != 0:
            raise ClusterError(
                f"postgres --version exited with status {process.returncode:d}: {stderr}"
            )

        version_string = stdout.decode("utf-8").strip(" \n")
        prefix = "postgres (PostgreSQL) "
        if not version_string.startswith(prefix):
            raise ClusterError(f"could not determine server version from {version_string!r}")
...
class HotStandbyCluster(TempCluster):
    ...
    def init(self, **settings):
        ...
        if process.returncode != 0:
            raise ClusterError(
                f"pg_basebackup init exited with status {process.returncode:d}:\n{output.decode()}"
            )
