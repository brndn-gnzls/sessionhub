        ...
        def call_con_method(self, *args, **kwargs):
            ...
            if self._con is None:
                raise exceptions.InterfaceError(
                    f"cannot call Connection.{meth_name}(): "
...
class PoolConnectionProxy(
    ...
    def __repr__(self):
        if self._con is None:
            return f"<{self.__class__.__name__} [released] {id(self):#x}>"
        else:
            return f"<{self.__class__.__name__} {self._con!r} {id(self):#x}>"
...
class Pool:
        ...
        **connect_kwargs,
        ...
        if not issubclass(record_class, protocol.Record):
            raise TypeError(
                ...
                f"asyncpg.Record, got {record_class!r}"
            )
    ...
    async def fetchmany(self, query, args, *, timeout=None, record_class=None):
        ...
        """
        async with self.acquire() as con:
            return await con.fetchmany(query, args, timeout=timeout, record_class=record_class)
